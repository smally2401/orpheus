...
// Generates `VALID_KEYS` (the list of key names `KeyCombo::from_key` checks
// against) and `key_string_to_key_name` (used elsewhere to turn a Slint
// `Key` back into one of these names) from this list of supported Slint
// `Key` variants. See `define_keys!` in `macros.rs`.
define_keys!(
    Backspace,
    Tab,
    Return,
